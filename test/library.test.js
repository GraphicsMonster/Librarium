...
contract('Library', async () => {
    ...
    beforeEach(async () => {
        ...
        LibraryInstance = await Library.new(BookToken.address);
    ...
    it('Should be able to register a user(Through library contract)', async () => {
        ...
        assert.equal(userDetails[0], "Devansh", "User name is not correct");
        assert.equal(userDetails[1], "dgupta0069@gmail.com", "User email is not correct");  
